...
function OneFolFolderForm(btnText, handleSubmit, projectData, serviceData){
    ...
    const [ennoblementTypes, setEnnobelementTypes] = useState([]);
    ...
    useEffect(() => {
        ...
        .then( resp => setEnnobelementTypes(resp.data))
        .catch( error => console.log(error));
    ...
    function handleOnSelectEnnoblement(e){
        const ennoblement = ennoblementTypes.filter(
            format => format.id === 
            e.target.options[e.target.selectedIndex].value
        );
    ...
    return (
        <form onSubmit={submit} className={styles.form}>  
            ...
            <Select id="ennoblementType" name="ennoblementType" text="Enobrecimento" options={ennoblementTypes} handleOnChange={handleOnSelectEnnoblement}
                value={ennoblementTypes ? ennoblementTypes.id : ''}
            />
...
export default OneFolFolderForm;